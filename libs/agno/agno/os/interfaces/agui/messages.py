"""Text extraction helpers for AG-UI messages and response chunks.

Three small functions:
- ``extract_agui_user_input``: pull the last user message from an AG-UI
  ``messages`` array (AG-UI frontends ship the full history on every request;
  the agent manages its own history server-side, so we only need the latest
  user message as input).
- ``extract_response_chunk_content``: pull rendered text out of an agent
  ``RunContentEvent``.
- ``extract_team_response_chunk_content``: pull rendered text out of a team
  ``TeamRunContentEvent``, recursively folding in member responses.
"""
