"""Agent + team event translation for the AG-UI interface.

Translates a single ``RunOutputEvent`` or ``TeamRunOutputEvent`` chunk into
the AG-UI events to emit. Workflow events are delegated to
``workflow.py`` so this file stays focused on the agent/team flow.

Public surface:
- ``create_events_from_chunk``: the main per-chunk translator.
- ``emit_event_logic``: post-emit bookkeeping (updates the EventBuffer's
  active/ended tool-call sets).
"""
...
from agno.os.interfaces.agui.workflow import (
    create_workflow_events_from_chunk,
    should_suppress_inner_workflow_event,
)
...
def create_events_from_chunk(
    ...
    Args:
        ...
        event_buffer: Event buffer tracking tool / reasoning / workflow state.
        run_state: Mutable dict reference to the agent's session state (for
            delta tracking).
        is_workflow: When True, drop inner agent/team content events to avoid
            duplication with WorkflowCompletedEvent.content emitted at
            completion.

    Returns:
        (events_to_emit, new_message_started_state, message_id)
    """
    events_to_emit: List[BaseEvent] = []
    is_content_event = False

    if should_suppress_inner_workflow_event(chunk, event_buffer, is_workflow):
    ...
    # Handle starting a new tool
        ...
        if chunk.tool is not None:  # type: ignore
            ...
            if message_started:
                events_to_emit.append(
                    TextMessageEndEvent(type=EventType.TEXT_MESSAGE_END, message_id=current_message_id)
                )

                # Set this message as the parent for any upcoming tool calls;
                # ensures multiple sequential tool calls share the same parent.
            ...
            if not parent_message_id:
                # Create parent message for tool calls without a preceding
                # assistant message (synthesize a tiny empty assistant turn).
            ...
            events_to_emit.append(
                ToolCallArgsEvent(
                    ...
                    # (datetime, UUID, bytes, etc.) commonly produced by tools.
    ...
    # Handle tool call completion
        ...
        if chunk.tool is not None:  # type: ignore
            ...
            if tool_call.tool_call_id not in event_buffer.ended_tool_call_ids:
                ...
                # Emit state delta after tool call completion (state may have
                # been mutated by the tool).
    ...
    # Delegate every workflow event to workflow.py. Terminal workflow events
    # (workflow_completed / workflow_error / workflow_cancelled) reach this
    # branch only when emitted out-of-band, since the streaming gate routes
    # them through completion.py — but workflow.py treats them as no-ops.
    elif chunk.event in {e.value for e in WorkflowRunEvent}:
        events_to_emit.extend(create_workflow_events_from_chunk(chunk, event_buffer))
    ...
    # Handle custom events
        ...
        # Use the complete Agno event as value if parsing it works, else fall
        # back to the content field and surface the to_dict failure so genuine
        ...
        events_to_emit.append(CustomEvent(name=custom_event_name, value=custom_event_value))
...
def emit_event_logic(event: BaseEvent, event_buffer: EventBuffer) -> List[BaseEvent]:
    ...
    Updates the EventBuffer's active/ended tool-call sets so subsequent
    chunks know which tool calls are still open.
    """
