"""Per-stream state for the AG-UI interface.

Holds:
- ``validate_agui_state``: coerces user-supplied AG-UI state into a plain dict.
- ``EventBuffer``: tracks message / tool-call / reasoning / workflow-progress
  state across a single AG-UI stream so we can satisfy AG-UI's event-ordering
  constraints (e.g. every ``TOOL_CALL_END`` has a matching ``TOOL_CALL_START``).
- ``create_state_delta_events``: computes JSON-Patch deltas against the last
  snapshot stored on the buffer.
"""
...
def validate_agui_state(state: Any, thread_id: str) -> Optional[Dict[str, Any]]:
    """Validate the given AG-UI state is of the expected type (dict)."""
    if state is None:
        return None
